...
public class BuildUpdateZip : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        GUILayout.Label("BuildResourcesFiles.json");
    ...
    private string TryGetLuaFrameworkFilesVersion()
    {
        ...
        foreach (var f in fs)
        {
            ...
            {
                var result = Regex.Match(f, ".*BuildResourcesFiles_(.*).json");
                if (null != result)
                {
    ...
    private List<string> GetNeedUpdateLuaList()
    {
        ...
        var originalLuaMD5FilePath = BuildUtils.BIN_PATH + "/BuildResourcesFiles_" + m_luaFrameworkFilesVersion + ".json";
