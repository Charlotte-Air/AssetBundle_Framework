...
处理Bundle依赖引用计数 与释放AssetBundle资源  </br>
加载资源的接口  </br>
...
-
⭐️ Update Process~</br>
`HotUpdate脚本` UpdateMode模式处理 `获取文件信息` -`检测更新` -`资源释放` -`进入游戏` </br>
`检测初次安装`：（一般最后安装完才写入filelist文件）只读取目录有热更资源，可读写目录是没有热更资源 （判断filelist文件是否存在）</br>
`检查更新阶段`：下载服务器资源的filelist文件，对比文件信息与本地一至文件路径 （知道相对路径就可以任意操作） </br>
`只读路径`: Application.streamingAssetsPath/XX/xxxx.ab </br>
`可读写路径`: Application.persistentDataPath/XX/xxxx.ab </br>
`资源服务器`: http://XXX.X.XXX/AssetBundles/XX/xxxx.ab </br>
